<template>
  ...
  <div class="row dr-builder-ui">
    <div class="col-md-2 col">
      ...
      <ul class="dr-byor-data-columns">
        <li v-for="field in availableFields" class="form-check form-check-inline" :key="field">
          <input
            id="flexCheckDefault"
            ...
            name="column"
            :value="field"
            :disabled="savedColumns.includes(selectedColumn)"
          >
    ...
    <div class="col-md-3 col">
      ...
      <ul class="dr-byor-data-columns">
        <li v-for="formatOption in Object.keys(formatOptions)" class="form-check form-check-inline" :key="formatOption">
          <input
            id="flexCheckDefault"
    ...
    <div class="col-md-3 col">
      ...
      <tbody class="dr-byor-data-columns">
        <tr v-for="(option, index) in selectedOptions" class="arrow-button-list" :key="index">
    ...
    <div class="col-md-3 col">
        ...
        <div class="form-inline">
          <div class="form-check column-format-label">
            <label class="form-check-label">
              <input v-model="newTab" class="form-check-input" type="checkbox">
              New tab
            </label>
          </div>
          <div class="form-check">
            <label class="form-check-label">
              <input v-model="asButton" class="form-check-input" type="checkbox">
              As button
            </label>
          </div>
        </div>
        <div v-if="asButton" class="form-inline">
      ...
      <div
        v-if="selectedOptions.includes('render background percent') ||
      ...
      <div v-if="selectedOptions.includes('fixed')">
        ...
        <div class="form-inline">
          <span>Place </span>
          <select v-model="fixedPlaces" class="form-control form-control-sm column-format-medium">
  ...
  <div class="row">
    <div class="col-md-12 col text-center dr-bubbles-wrapper">
      <div v-for="column in savedColumns" class="dr-format-bubble" :key="column">
        <span class="name">{{ column }}</span> |
        <span class="type">{{ allColumnsConfig[column]['type'][0] }}</span>
        <span v-if="selectedColumn === column" class="editing">Editing</span>
...
watch([availableFields, fieldNameOld], (newValues, oldValues) => {
  ...
  if (allColumnsConfig.value[fieldNameOld.value]) {
    const updatedFieldConfig = JSON.parse(JSON.stringify(allColumnsConfig.value[fieldNameOld.value])); // Deep copy
...
function moveUpDown (index, down = false) {
  if (down) { index++; }
  const risingItem = selectedOptions.value[index];
...
function saveColumn () {
  ...
  const columnConfig = JSON.parse(JSON.stringify(singleColumnConfig.value)); // Deep copy
...
function editColumn (column) {
  const loadConfig = JSON.parse(JSON.stringify(allColumnsConfig.value[column])); // Deep copy
  clearAll();
  selectedColumn.value = column;
  singleColumnConfig.value = loadConfig;
  for (let i = 0; i < loadConfig.type.length; i++) {
    const item = loadConfig.type[i];
