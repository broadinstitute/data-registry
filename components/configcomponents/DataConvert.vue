<template>
  <div>
    ...
    <div class="row dr-builder-ui">
      <div class="col-md-3 col">
        ...
        <select v-model="dataConvertType" class="form-control" :disabled="editingFieldIndex >= 0">
          <option value="">
            Select a data convert type
          </option>
          <option v-for="dcOption in dataConvertOptions" :value="dcOption.selectValue" :key="dcOption">
  ...
  <div class="row">
    <div class="col-md-12 col text-center dr-bubbles-wrapper">
      <div v-for="(field, index) in savedFieldConfigs" class="dr-format-bubble" :key="index">
        <span class="name">{{ field["field name"] }}</span>
        <span class="type">{{ field["type"] }}</span>
        <span v-if="editingFieldIndex === index" class="editing">Editing</span>
...
function editField (index) {
  ...
  const savedField = savedFieldConfigs.value[index];
...
function saveField () {
  ...
  const newField = JSON.parse(JSON.stringify(currentFieldConfig.value)); // Deep copy
  if (!newField["field name"]) {
    failedSaveMsg = "Enter a field name.";
    showMsg.value = true;
    return;
  }
  const nameCheck = fieldNameCheck(newField);
  if (!nameCheck.ready) {
  ...
  if (editingFieldIndex.value === -1) {
    ...
    const oldField = savedFieldConfigs.value[editingFieldIndex.value];
    if (!oldField["create new"] && !!newField["create new"]) {
      store.restoreUnConverted(newField["raw field"]);
    }
    const oldName = oldField["field name"];
    const newName = newField["field name"];
...
function fieldNameOkay (fieldName, rawField = "", createNew = true) {
  ...
  for (const unConvertedField of unConvertedFields.value) {
    const duplicateIsOk = (unConvertedField["raw field"] === rawField && !createNew);
  ...
  for (let i = 0; i < savedFieldConfigs.value.length; i++) {
    ...
    const existingField = savedFieldConfigs.value[i];
    const moreNames = existingField.type === 'split' ? existingField["field name"] : [existingField["field name"]];
...
function deleteField () {
  ...
  const fieldDeleted = savedFieldConfigs.value[editingFieldIndex.value];
...
watch(columnNameChange, () => {
  const rawField = columnNameChange.value[0];
  const newName = columnNameChange.value[1];
  for (let i = 0; i < savedFieldConfigs.value.length; i++) {
    const fieldConfig = savedFieldConfigs.value[i];
    const oldName = fieldConfig["field name"];
