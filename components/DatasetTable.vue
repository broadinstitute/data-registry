            ...
            <div class="card">
                <div class="card-header">
                    ...
                    >
                        ...
                        <Column
                            field="global_sample_size"
                            header="Samples"
                        ></Column>
                        <Column field="ancestry" header="Ancestry"></Column>
                        <Column header="Sex">
                            <template #body="{ data }">
                                <span v-html="formatSex(data.sex)"
                            /></template>
                        </Column>
                        <Column field="data_type" header="Data Type"></Column>
                        <Column
                            field="data_submitter"
                            header="Submitter"
                        ></Column>
                        <Column
                            header="Date"
                ...
                <tr>
                    <td>
                        <a
                            class="link-primary"
                            @click="
                                route.push({ path: `/datasets/${dataset.id}` })
                            "
                            >{{ dataset.name }}</a
                        >
                    </td>
                    ...
                    <td v-if="dataset.publicly_available">
                        <button
                            ...
                            @click="toggleFiles(dataset)"
                        >
                            {{ dataset.showFiles ? "Hide" : "Show" }} Files
                        </button>
                    </td>
                    <td v-else>Not Shared</td>
                    <td>
                        {{
                            dataset.created_at
...
async function toggleFiles(dataSet) {
    if (!dataSet.files) {
        ...
        for (const file of dataSet.files) {
            file.phenotype = store.phenotypes[file.phenotype]
                ? store.phenotypes[file.phenotype].description
                : file.phenotype;
            file.copyMsg = "";
        }
    }
